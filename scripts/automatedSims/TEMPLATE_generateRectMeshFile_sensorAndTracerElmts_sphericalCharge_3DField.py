...
#Pull parameters from sampleParamSpace.py
def load_parameters_from_sampleParamSpace():
    try:
        current_dir = Path(__file__).parent
        #Search in current directory and parent directory
        for search_dir in [current_dir, current_dir.parent]:
            sps_path = search_dir / "sampleParamSpace.py"
            if sps_path.exists():
                sys.path.insert(0, str(search_dir))
                import sampleParamSpace5 as sps
                return {
                    'ELEMENT_SIZE_CM': sps.ELEMENT_SIZE_CM,
                    'EXPL_RADIUS_CM': sps.EXPL_RADIUS_CM,
                    'REGION_X_DIM': sps.REGION_X_DIM,
                    'REGION_Y_DIM': sps.REGION_Y_DIM,
                    'REGION_Z_DIM': sps.REGION_Z_DIM,
                    'CENTERLINE_TOL': sps.CENTERLINE_TOL if hasattr(sps, 'CENTERLINE_TOL') else 1.0e-8,
                }
    except Exception as e:
        print(f"Warning: Could not import from sampleParamSpace.py: {e}")
    return None

params_from_sps = load_parameters_from_sampleParamSpace()

#Default values (will be overridden by sampleParamSpace.py if available)
_DEFAULT_ELEMENT_SIZE_CM = 0.2
_DEFAULT_EXPL_RADIUS_CM = 16.0
_DEFAULT_REGION_X_DIM = 36
_DEFAULT_REGION_Y_DIM = 36
_DEFAULT_REGION_Z_DIM = 36
...
ELEMENT_SIZE_CM = params_from_sps['ELEMENT_SIZE_CM'] if params_from_sps else _DEFAULT_ELEMENT_SIZE_CM
EXPL_RADIUS_CM  = params_from_sps['EXPL_RADIUS_CM'] if params_from_sps else _DEFAULT_EXPL_RADIUS_CM
REGION_X_DIM    = params_from_sps['REGION_X_DIM'] if params_from_sps else _DEFAULT_REGION_X_DIM
REGION_Y_DIM    = params_from_sps['REGION_Y_DIM'] if params_from_sps else _DEFAULT_REGION_Y_DIM
REGION_Z_DIM    = params_from_sps['REGION_Z_DIM'] if params_from_sps else _DEFAULT_REGION_Z_DIM

#Centerline tolerance: scale with element size (elements spanning y=0, z=0 can have nodes up to element_size away)
#Use 1.1x element size to ensure we capture all elements at the centerline
CENTERLINE_TOL  = 1.1 * ELEMENT_SIZE_CM
...
#Only includes elements where ALL nodes are on the centerline (y≈0 AND z≈0)
def define_tracer_elements(node_section, element_section):
    ...
    for row in element_section:
        ...
        y_coords = [nodeID_to_coords[n][1] for n in nids]
        z_coords = [nodeID_to_coords[n][2] for n in nids]
        # Check that ALL 8 nodes are on the centerline (all y≈0 AND all z≈0)
        all_on_centerline = all(abs(y) <= tol for y in y_coords) and all(abs(z) <= tol for z in z_coords)
        if all_on_centerline:
...
def format_sections_into_file(node_section, element_section, sensor_elements, sensor_set_id,
    ...
    #print(f"DEBUG: Sensor elements found: {len(sensor_elements)}")
    #print(f"DEBUG: Tracer elements found: {len(tracer_elements)}")
    #print(f"DEBUG: Tracer nodes found: {len(tracer_nodes)}")
