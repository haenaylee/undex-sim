...
# Assumptions: mesh's bottom left corner is at the origin, thin thickness of z=1 element, inputs except element_size and expl_radius are integers, 
...
def generate_nodes(element_size, outer_dims):
    ...
    if abs(zf-element_size) > tol:
        print("The z dimension is not equal to the element size, please set them equal to each other.")
        return None
...
if __name__ == '__main__':
    ...
    outer_dims = get_tuple("Enter outer region dimensions - make sure the z dimension = the element size (e.g., 32 32 0.16): ")
    expl_radius = float(input("Enter radius of spherical explosive charge: "))

    fixed_coords = []
    fixed_coords_input = input("Enter fixed node coordinates, separated by commas (e.g., 0 0 0, 0 0 0.16): ").strip()
